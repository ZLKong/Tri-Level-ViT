    ...
    def forward(self, x, ratio, attn_ratio):
        ...
        for i, blk in enumerate(self.blocks):
            if i in self.pruning_loc:           # i = 3, 6, 9
                x, token_idx, attn_mask = blk(x, token_keep_ratio= ratio, token_idx=token_idx, token_prune=True, attn_prune=True, attn_mask=attn_mask, attn_keep_ratio=attn_ratio)
            else:
                x, token_idx, attn_mask = blk(x, token_keep_ratio=ratio, token_idx=token_idx, token_prune=False, attn_prune=False,  attn_mask=attn_mask, attn_keep_ratio=attn_ratio)
