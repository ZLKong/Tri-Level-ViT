...
def train_one_epoch(model: torch.nn.Module, criterion: DistillationLoss,
    ...
    for samples, targets, idx in metric_logger.log_every(data_loader, print_freq, header):
        ...
        #print('idx',idx)  #tensor([536147, 536147, 536147, 706066], device='cuda:0')
        targets_org=targets
        ...
        with torch.cuda.amp.autocast():#(enabled=False):
            outputs, cls_attn = model(samples, ratio=token_ratio, attn_ratio=attn_ratio)
            loss = criterion(samples, outputs, targets)

        # >>>>>>>>  data sparse  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        outputs_pred = outputs
        #print('outputs_pred',outputs_pred)
        _, predicted = torch.max(outputs_pred.data, 1) #max value in each row (token), total of B values, 
        cls_var = torch.var(cls_attn,1)  #add cls var
        ...
        for j, index in enumerate(idx):
            ...
            index_stats[3].append(cls_var[j].sum().item())
...
def evaluate(data_loader, model, device, args):
    ...
    for images, target, idx in metric_logger.log_every(data_loader, 10, header):
        ...
        with torch.cuda.amp.autocast():
            output, cls_attn = model(images, ratio=args.keep_ratio, attn_ratio=args.attn_ratio)
