...
def build_loader(config):
    ...
    train_example_idx = random_shuffle[:1281167 - 128117]
    removed_example_idx = random_shuffle[1281167 - 128117:]
    ...
    dataset_val = MyValDataset(is_train=False, config=config)
    dataset_remove = MyTrainDataset(is_train=True, config=config, train_example_idx=removed_example_idx)
    ...
    if config.DATA.ZIP_MODE and config.DATA.CACHE_MODE == 'part':
        ...
        #indices_remove = np.arange(dist.get_rank(), len(dataset_remove), dist.get_world_size())
        #sampler_remove = SubsetRandomSampler(indices_remove)
    else:
    ...
    return dataset_train, dataset_val, data_loader_train, data_loader_val, mixup_fn, dataset_remove, train_example_idx, removed_example_idx


def build_dataset(is_train, config):
    transform = build_transform(is_train, config)
    if config.DATA.DATASET == 'imagenet':
        prefix = 'train' if is_train else 'val' #'imagenetv2-matched-frequency-format-val' #'val'
...
class MyValDataset(Dataset):
    def __init__(self, is_train, config):
        ...
        if config.DATA.DATASET == 'imagenet':
            root = os.path.join(config.DATA.DATA_PATH, 'train' if is_train else 'val')
            self.val_dataset = datasets.ImageFolder(root, transform=transform)
            #self.val_dataset = datasets.ImageFolder(root='imagenetv2-matched-frequency', transform=transform)

    def __getitem__(self, index):
        data, target = self.val_dataset[index]
        # Your transformations here (or set it in CIFAR10)
        return data, target, index#, nb_classes
    def __len__(self):
